...
export default function transclude(this: Processor): void {
  ...
  const vaultFiles = (this as unknown as { data: Record<string, unknown> }).data["vaultFiles"] as Record<string, string> | undefined
  const files: Record<string, string> | null = vaultFiles ?? null
  this.parser = (document, file) => {
    const seen = new Set<string>()
    const root = findNoteFileByBody(document, files)
    if (root !== null) {
      seen.add(root)
    }
    return parse(resolveTransclusions(document, seen, files), file)
  }
}