# Processor — Pure Markdown → HTML/PDF

## Why This Package Exists

All vault-to-static transformation logic lives here. It has zero Cloudflare dependencies and can be tested/debugged in plain Node.

## Responsibility

- Convert Obsidian-flavored markdown to HTML and PDF
- Handle frontmatter, callouts, transclusions, wikilinks, embedded images
- Read and inline CSS snippets in deterministic order
- Provide pure functions: `processObsidianMdToHtml`, `processObsidianMdToPdf`, `processVaultToStatic`
...
- `src/processObsidianMdToPdf.ts`: input md, output pdf
- `src/processVaultToStatic.ts`: orchestrating the above
...
- `src/plugin/transclude.ts`: pure `vaultFiles` param; no global, no `VAULT_ROOT` FS outside `processVaultToStatic.ts`/`bundleVaultIntoWorker.ts`
